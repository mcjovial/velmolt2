...
class DepositController extends Controller
{
    ...
    public function createManual()
    {
        ...
        return view('admin.deposit.create_manual', $data);
    }
    
    public function manual()
    {
        $data['title'] = 'Payment gateways';
        $data['gateway']= Manual::orderBy('id', 'DESC')->get();
        $data['deposit']= Manualtransfer::orderBy('id', 'DESC')->get();
        return view('admin.deposit.manual', $data);
    }
    ...
    public function storeManual(Request $request)
    {
        ...
        if($request->hasFile('image')){
            ...
            $filename = 'coin_'.time().'.'.$image->extension();
            $location = 'asset/manual/' . $filename;
    ...
    {
        $data = Manual::findOrFail($request->id);
        $data->name=$request->name;
        $data->wallet=$request->wallet;
        $data->wallet_id=$request->wallet_id;
        if($request->hasFile('image')){
    ...
    {
        $data = Manualtransfer::findOrFail($id);
            $res =  $data->delete();
            if ($res) {
                return back()->with('success', 'Request was Successfully deleted!');
            } else {
    ...
    public function approvemt($id)
    {
        ...
        if($set->email_notify==1){
            send_email(
                $user->email, 
                $user->username, 
                $data->coin.' Deposit Request has been approved', 
                $data->coin.' Deposit worth of '.$currency->symbol.$data->amount.'. has been approved<br>Thanks for working with us.'
            );
        }
            $audit['user_id']=$data->user_id;
            $audit['trx']=str_random(16);
            $audit['log']=$data->coin.' Deposit request Approved '.$data->trx;
            Audit::create($audit);  
        if ($res) {
            return back()->with('success', 'Request was Successfully approved!');
        } else {
    ...
    public function declinemt($id)
    {
        ...
        if($set->email_notify==1){
            send_email(
                $user->email, 
                $user->username, 
                $data->coin.' Deposit Request has been declined', 
                $data->coin.' Deposit worth of '.$currency->symbol.$data->amount.'. has been declined<br>Thanks for working with us.'
            );
        }
        $audit['user_id']=$data->user_id;
        $audit['trx']=str_random(16);
        $audit['log']= $data->coin.' Deposit request Declined '.$data->trx;
        Audit::create($audit); 
        if ($res) {
            return back()->with('success', 'Request was Successfully declined!');
        } else {
